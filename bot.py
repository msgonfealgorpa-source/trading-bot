...
class Tracker:
    ...
    def send(self, m):
        try: 
            requests.post(f"https://api.telegram.org/bot{self.T}/sendMessage", data={'chat_id': self.C, 'text': m, 'parse_mode': 'Markdown'})
        except: 
            pass
...
class FuturesBot:
    ...
    def run(self):  
        self.send("🚀 *الوحش V6.1 (معدل) جاهز!*")  
        print("\n🚀 بدء المحرك الثلاثي...")  
        while True:  
