"""
═══════════════════════════════════════════════════════════════════════
  🔥 القناص الأسطوري V6.0 — Legendary Sniper Reborn 🔥
═══════════════════════════════════════════════════════════════════════
  بوت متكامل: مسح شامل + تصفية خارجية + تحليل مزدوج + تنفيذ ذكي

  ✅ مسح جميع أزواج USDT (تدريجي آمن — لن يُحظر أبداً)
  ✅ تصفية خارجية: CoinGecko + DexScreener ← ثم طلبات محددة لبينانس
  ✅ تحليل مزدوج: SMC + 7 مؤشرات تقليدية (تأكيد مزدوج)
  ✅ كشف إعلانات بينانس (جديدة + محذوفة)
  ✅ كشف ارتفاعات الحجم (ضخ أموال ذكي)
  ✅ مؤشر الخوف والطمع
  ✅ نظام Weight Tracker يمنع تجاوز حدود بينانس
  ✅ وقف خسارة متحرك + إدارة مخاطر صارمة
  ✅ WebSocket للأسعار اللحظية + REST احتياطي
  ✅ قاعدة بيانات SQLite للصفقات والإحصائيات
═══════════════════════════════════════════════════════════════════════
"""
...
class WeightTracker:
    """
    يتتبع وزن طلبات بينانس لضمان عدم تجاوز الحد.
    الحد الآمن: 2400 وزن/دقيقة (40% من حد بينانس 6000)
    """
    def __init__(self, max_per_minute=2400):
        self.max_weight = max_per_minute
        self.window = []  # [(timestamp, weight), ...]
    ...
    async def wait_for_capacity(self, weight):
        """ينتظر حتى يتوفر وزن كافي للطلب"""
        waited = 0
        while not self.can_request(weight):
            await asyncio.sleep(1)
            waited += 1
            if waited > 65:  # أكثر من دقيقة = مشكلة
                self._cleanup()  # تنظيف إجباري
                break
        self.window.append((time.time(), weight))

    def update_from_headers(self, headers):
        """تحديث من رؤوس استجابة بينانس الفعلية"""
        try:
...
class LegendarySniperBotV6:
    def __init__(self):
        ...
        # ─── بيانات السوق ───
        self.all_usdt_pairs = []        # جميع أزواج USDT
        self.known_symbols = set()      # أسماء الأزواج المعروفة
        self.step_sizes_cache = {}      # حجم خطوة كل زوج
        self.live_prices = {}           # أسعار لحظية
        self.live_klines = {}           # شموع لحظية
        ...
        # ─── قائمة العملات الساخنة (من مصادر خارجية) ───
        self.hot_coins = set()          # عملات رائجة من CoinGecko/DexScreener
        self.volume_spikes = []         # عملات بحجم مشبوه

        # ─── كاش التحليل (منع إعادة التحليل المتكرر) ───
        self.analysis_cache = {}        # symbol → {'result': dict, 'time': float}
        self.CACHE_TTL = 300            # 5 دقائق صلاحية الكاش
        ...
        for attempt in range(retries):
            try:
                ...
                async with self.session.request(method, url, params=req_params,
                    ...
                    # تحديث متتبع الوزن من الرأس الفعلي
                    ...
                    if r.status == 200:
                        ...
                        wait = 10 * (attempt + 1)
                        logger.warning(f"⏳ Rate limited! انتظار {wait} ثانية...")
                        await asyncio.sleep(wait)
                    elif r.status in [500, 502, 503]:
                        await asyncio.sleep(2 ** attempt)
                    else:
                        text = await r.text()
                        logger.debug(f"Binance {r.status}: {text[:200]}")
                        return None
            except asyncio.TimeoutError:
                await asyncio.sleep(2 ** attempt)
            except Exception as e:
                logger.error(f"طلب بينانس خطأ: {e}")
                await asyncio.sleep(2 ** attempt)
        return None
    ...
    # ═══════════════════════════════════════════════════
    #       وحدة تحميل بيانات السوق الشاملة
    # ═══════════════════════════════════════════════════
    async def load_market_data(self):
        """تحميل جميع أزواج USDT + أحجام الخطوات"""
        ...
        if self.known_symbols:
            ...
            if newly_listed:
                ...
                await self.tg(msg)
                # إضافتها للعملات الساخنة
                for sym in newly_listed:
                    base = sym.replace('USDT', '')
                    self.hot_coins.add(base)

        self.all_usdt_pairs = new_pairs
        self.known_symbols = new_symbols
        logger.info(f"📊 تم تحميل {len(self.all_usdt_pairs)} زوج USDT")
    ...
    # ═══════════════════════════════════════════════════
    #     وحدة الاتصال اللحظي (WebSocket + REST)
    # ═══════════════════════════════════════════════════
    async def ws_manager(self):
        """WebSocket للأسعار والشموع اللحظية"""
        streams = []
        ...
        while True:
            try:
                async with websockets.connect(ws_url, ping_interval=20) as ws:
                    logger.info("✅ WebSocket متصل (أسعار + شموع لحظية)!")
                ...
                if "404" in str(e) or "403" in str(e) or "rejected" in str(e):
                    logger.warning("⚠️ WebSocket غير متاح، تحول لـ REST Polling...")
                    await self.rest_poller()
                    break
                await asyncio.sleep(5)

    async def rest_poller(self):
        """بديل REST عند فشل WebSocket"""
    ...
    # ═══════════════════════════════════════════════════
    #   التحليل التقليدي (7 مؤشرات من V2.0)
    # ═══════════════════════════════════════════════════
    def _analyze_indicators(self, df, direction):
        """
        تحليل 7 مؤشرات تقليدية على DataFrame
        يرجع: (score, signals_list)
        """
        score = 0
        signals = []
        ...
        try:
            ...
            if direction == 'BUY':
                if rsi_val < 25:
                    ...
                    score -= 3; signals.append(f"⚠️ RSI مرتفع ({rsi_val:.0f})")
            else:  # SELL
                if rsi_val > 75:
                    ...
                    score -= 3; signals.append(f"⚠️ RSI منخفض ({rsi_val:.0f})")
            ...
            if direction == 'BUY':
                if ema9.iloc[-1] > ema21.iloc[-1] and ema9.iloc[-2] <= ema21.iloc[-2]:
                    score += 2; signals.append("📈 EMA9 تقاطع فوق EMA21")
                ...
                if ema9.iloc[-1] < ema21.iloc[-1] and ema9.iloc[-2] >= ema21.iloc[-2]:
                    score += 2; signals.append("📉 EMA9 تقاطع تحت EMA21")
                if price < ema50.iloc[-1]:
                    score += 1
                else:
                    score -= 1
            ...
            if direction == 'BUY':
                if price <= bb_lower:
                    score += 3; signals.append("📈 لامس البولنجر السفلي (ارتداد!)")
                elif price <= bb_lower * 1.01:
                    score += 1; signals.append("📈 قريب من البولنجر السفلي")
            else:
                if price >= bb_upper:
                    score += 3; signals.append("📉 لامس البولنجر العلوي (تصحيح!)")
    ...
    # ═══════════════════════════════════════════════════
    #    التحليل المزدوج الشامل (SMC + مؤشرات)
    # ═══════════════════════════════════════════════════
    async def analyze_coin_full(self, symbol):
        """
        تحليل شامل: SMC + 7 مؤشرات تقليدية
        مع فلتر الفريم الأكبر (HTF 1h)
        """
        ...
        try:
            # ═══ 1. تحليل الفريم الأكبر (1h) — تحديد الاتجاه العام ═══
            ...
            if not htf_trend: return None  # لا اتجاه واضح
            ...
            # ═══ 4. تحليل المؤشرات التقليدية (7 مؤشرات) ═══
            ind_score, ind_signals = self._analyze_indicators(df_15m, result['direction'])
            # تحويل نقاط المؤشرات لتتناسب مع اتجاه الصفقة
            if result['direction'] == 'SELL':
                ind_score = -ind_score  # عكس النقاط للبيع
    ...
    # ═══════════════════════════════════════════════════
    #     رادار إعلانات بينانس (جديدة + محذوفة)
    # ═══════════════════════════════════════════════════
    async def check_binance_announcements(self):
        """مراقبة إعلانات بينانس — كشف الإدراج الجديد والمحذوف"""
        try:
            ...
            logger.error(f"خطأ فحص الإعلانات: {e}")

    # ═══════════════════════════════════════════════════
    #     رادار CoinGecko (العملات الرائجة + الأكثر ارتفاعاً)
    # ═══════════════════════════════════════════════════
    async def scan_coingecko_trending(self):
        """البحث عن العملات الرائجة على CoinGecko → تصفية لبينانس"""
        trending_on_binance = []
        try:
            ...
            logger.error(f"خطأ CoinGecko Trending: {e}")

        return trending_on_binance

    async def scan_coingecko_gainers(self):
        """البحث عن الأكثر ارتفاعاً → تصفية لبينانس فقط"""
        gainers = []
        try:
            ...
            if binance_gainers:
                ...
                for i, g in enumerate(binance_gainers[:7]):
                    msg += (f"{i+1}. 🪙 *{g['name']}* (`{g['symbol']}`)\n"
                           f"   💰 ${g['price']:.4f} | 📈 +{g['change']:.1f}%\n\n")
                await self.tg(msg)
                gainers = [g['symbol'] for g in binance_gainers]

        except Exception as e:
            logger.error(f"خطأ CoinGecko Gainers: {e}")

        return gainers

    # ═══════════════════════════════════════════════════
    #     رادار DexScreener (عملات DEX الساخنة)
    # ═══════════════════════════════════════════════════
    async def scan_dexscreener_hot(self):
        """اكتشاف العملات الساخنة على DEX (قد تُدرج على بينانس)"""
        try:
            ...
            msg += "(قد تُدرج على بينانس قريباً!)\n━━━━━━━━━━━━━━━━━━━━━━━━\n"

            for i, pair in enumerate(pairs[:5]):
                base = pair.get('baseToken', {}).get('symbol', 'N/A')
                name = pair.get('baseToken', {}).get('name', '')
                price_usd = pair.get('priceUsd', '0')
                change = pair.get('priceChange', {}).get('h24', 0) or 0
                dex = pair.get('dexId', '')
                on_binance = f"{base.upper()}USDT" in self.known_symbols
                icon = "✅" if on_binance else "🔄"

                msg += (f"{i+1}. 🪙 *{name}* (`{base}`) {icon}\n"
                       f"   💰 ${float(price_usd):.6f} | 📈 {change:+.1f}% | DEX: {dex}\n\n")

                if on_binance:
                    self.hot_coins.add(base.upper())

            msg += "✅ = على بينانس | 🔄 = قد تُدرج قريباً"
            await self.tg(msg)

        except Exception as e:
            logger.error(f"خطأ DexScreener: {e}")
        return []
    ...
    async def check_fear_greed(self):
        """فحص مؤشر الخوف والطمع للسوق ككل"""
        try:
            ...
            logger.error(f"خطأ Fear & Greed: {e}")
        return 50

    # ═══════════════════════════════════════════════════
    #     كشف ارتفاعات الحجم (طلب واحد فقط!)
    # ═══════════════════════════════════════════════════
    async def detect_volume_spikes(self):
        """كشف العملات بحجم غير معتاد — باستخدام طلب واحد (40 وزن فقط)"""
        spikes = []
        try:
            # طلب واحد يجلب كل الأزواج! (40 وزن)
            ...
            for ticker in data:
                ...
                # حجم عالي + صعود معتدل = تراكم (ذكاء أموال)
                if quote_vol > 5000000 and 2 < price_change < 20 and trades > 1000:
                    ...
                    # إضافة للعملات الساخنة
                    base = symbol.replace('USDT', '')
                    self.hot_coins.add(base)

            spikes.sort(key=lambda x: x['volume_usdt'], reverse=True)
            self.volume_spikes = spikes  # حفظ للمرجع
            ...
            if spikes[:7]:
                ...
                for i, s in enumerate(spikes[:7]):
                    ...
                    msg += (f"{i+1}. 🪙 `{s['symbol']}`\n"
                           f"   💧 حجم: ${vol_m:.1f}M | 📈 {s['change_pct']:+.1f}%"
                           f" | 🔄 {s['trades']:,} صفقة\n\n")
                await self.tg(msg)

        except Exception as e:
            logger.error(f"خطأ كشف الحجم: {e}")

        return spikes

    # ═══════════════════════════════════════════════════
    #  🎯 المسح الذكي بالتصفية الخارجية (قلب النظام)
    # ═══════════════════════════════════════════════════
    async def smart_scan_with_external_filter(self):
        """
        المسح الذكي: المصادر الخارجية ← تصفية ← طلبات محددة لبينانس
        هذا هو النظام الذي يقلل طلبات بينانس للحد الأدنى
        """
        binance_targets = set()

        # 1. CoinGecko Trending → تصفية لبينانس فقط
        try:
            url = "https://api.coingecko.com/api/v3/search/trending"
            async with self.session.get(url, timeout=10) as r:
                if r.status == 200:
        ...
        await asyncio.sleep(1)  # احترام حدود CoinGecko

        # 2. CoinGecko Gainers → تصفية لبينانس فقط
        try:
            url = "https://api.coingecko.com/api/v3/coins/markets"
        ...
        # 3. DexScreener → فقط العملات الموجودة على بينانس
        try:
            url = "https://api.dexscreener.com/latest/dex/search?q=trending"
            async with self.session.get(url, timeout=10) as r:
                if r.status == 200:
        ...
        # 4. إضافة العملات من ارتفاعات الحجم
        for spike in self.volume_spikes[:10]:
            binance_targets.add(spike['symbol'])

        # 5. إضافة العملات الساخنة المخزنة
        for coin in self.hot_coins:
            sym = f"{coin}USDT"
            if sym in self.known_symbols:
                binance_targets.add(sym)

        # 6. إضافة العملات ذات الأولوية
        for coin in self.priority_coins:
            sym = f"{coin}USDT"
            if sym in self.known_symbols:
                binance_targets.add(sym)

        # ═══ الآن: تحليل فقط الأهداف المحددة على بينانس! ═══
        logger.info(f"🎯 مسح ذكي: {len(binance_targets)} هدف مُصفّى من المصادر الخارجية")

        results = []
        targets_list = list(binance_targets)

        for i, symbol in enumerate(targets_list):
            ...
            # تأخير بين كل تحليل (4 وزن × تحليل)
            await asyncio.sleep(0.5)

            # توقف كل 15 عملة لضمان عدم تجاوز الوزن
            if (i + 1) % 15 == 0:
        ...
        if sells:
            ...
            for i, a in enumerate(sells):
                msg += f"{i+1}. 🪙 `{a['symbol']}` | نقاط: *{a['score']}* | ${a['price']:.4f}\n"
            await self.tg(msg)

        self.stats['total_scans'] += len(targets_list)
        return results

    # ═══════════════════════════════════════════════════
    #     المسح الشامل المحصّن (تدريجي ومتتابع)
    # ═══════════════════════════════════════════════════
    async def full_market_scan(self):
        """
        فحص جميع أزواج USDT بشكل تدريجي ومتتابع
        يستخدم pre-filter لتقليل عدد طلبات klines
        """
        ...
        # ═══ الخطوة 1: Pre-filter بطلب واحد (40 وزن فقط!) ═══
        ...
        for ticker in all_tickers:
            ...
            # تصفية ذكية: سيولة كافية + حركة معقولة
            if quote_vol > 500000 and -30 < price_change < 30 and trades > 200:
                # نقاط أهمية (للترتيب)
                interest_score = 0
                base = symbol.replace('USDT', '')

                # عملات ساخنة من المصادر الخارجية = أولوية قصوى
        ...
        for chunk_start in range(0, len(top_candidates), CHUNK_SIZE):
            ...
            # توقف بين كل مجموعة (حماية من الحظر)
            await asyncio.sleep(3)

            # تقرير مرحلي كل 30 عملة
            if (chunk_start + CHUNK_SIZE) % 30 == 0:
                logger.info(f"📊 تم فحص {scanned}/{len(top_candidates)} — "
                           f"وزن مستخدم: {self.weight_tracker.current}")
    ...
    # ═══════════════════════════════════════════════════
    #     الفحص السريع (أولوية + عملات ساخنة)
    # ═══════════════════════════════════════════════════
    async def quick_scan(self):
        """فحص سريع للعملات ذات الأولوية + الساخنة فقط"""
        targets = set()

        # إضافة العملات ذات الأولوية
        for coin in self.priority_coins:
            sym = f"{coin}USDT"
            if sym in self.known_symbols and sym in self.step_sizes_cache:
                targets.add(sym)

        # إضافة العملات الساخنة
        for coin in self.hot_coins:
            sym = f"{coin}USDT"
            if sym in self.known_symbols and sym in self.step_sizes_cache:
                targets.add(sym)
        ...
        found = []
        for symbol in list(targets)[:40]:  # حد أقصى 40 عملة
            if symbol in self.active_trades: continue
    ...
    async def execute_trade(self, analysis):
        ...
        if status in ['FILLED', 'PARTIALLY_FILLED']:
            ...
            all_signals = smc_signals + ind_signals

            msg = (f"✅ *صفقة {side} منفذة!*\n━━━━━━━━━━━━━━━━━━━━━━━━\n"
                   f"🪙 الزوج: `{symbol}`\n"
                   f"💵 سعر الدخول: `{fill_price:.6f}`\n"
                   f"📊 الكمية: `{total_qty}`\n"
                   f"💰 المبلغ: `${total_cost:.2f}`\n"
                   f"🛑 وقف الخسارة: `{analysis['sl']:.6f}`\n"
                   f"🎯 هدف الربح: `{analysis['tp']:.6f}`\n"
    ...
    # ═══════════════════════════════════════════════════
    #     مراقبة الصفقات المفتوحة (مع وقف متحرك)
    # ═══════════════════════════════════════════════════
    async def monitor_trades(self):
        ...
        for symbol, reason, close_price in symbols_to_close:
            ...
            msg = (f"🏁 *إغلاق `{symbol}`*\n━━━━━━━━━━━━━━━━━━━━━━━━\n"
                   ...
                   f"💵 الإغلاق: `{fill_price:.6f}`\n"
                   f"{icon} النتيجة: `{pnl:.2f} USDT`\n"
                   f"🏆 الإجمالي: {self.stats['wins']}W / {self.stats['losses']}L")
            await self.tg(msg)

    # ═══════════════════════════════════════════════════
    #     مزامنة الصفقات مع بينانس
    # ═══════════════════════════════════════════════════
    async def sync_with_binance(self):
        """حذف الصفقات الميتة من الذاكرة"""
        ...
        for sym in active_symbols:
            ...
            # إذا كانت صفقة قديمة جداً (أكثر من 24 ساعة)
    ...
    # ═══════════════════════════════════════════════════
    #        🧠 حلقة التشغيل الرئيسية (الدماغ)
    # ═══════════════════════════════════════════════════
    async def main_loop(self):
        """حلقة التشغيل — تتحكم بكل المهام بتدفق هادئ"""
        ...
        # رسالة التشغيل
        mode = "⚔️ تداول تلقائي" if self.TRADE_ENABLED else "👁️ مراقبة فقط"
        msg = ("🔥 *القناص الأسطوري V6.0 — بدأ العمل!*\n━━━━━━━━━━━━━━━━━━━━━━━━\n"
               f"📡 الوضع: {mode}\n"
               ...
               "🎯 المسح الذكي (تصفية خارجية): كل 30 دقيقة\n"
            ...
            while True:
                try:
                    ...
                    if now - self.timers['full_scan'] > 7200:
                        await self.load_market_data()  # تحديث الأزواج
