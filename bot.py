...
class LegendarySniperBotV3:
    ...
    async def analyze_coin(self, symbol):
        try:
            ...
            is_bullish_trend = df_4h.iloc[-1]['close'] > ema50_calc.iloc[-1]
            ...
            if is_bullish_trend and rsi_val < 35: result['score'] += 2; result['signals'].append("RSI Oversold")
            macd_ind = ta.trend.MACD(df_15m['close'])
            if macd_ind.macd().iloc[-1] > macd_ind.macd_signal().iloc[-1] and macd_ind.macd().iloc[-2] <= macd_ind.macd_signal().iloc[-2]: result['score'] += 2; result['signals'].append("MACD Cross")
            vol_avg = df_15m['volume'].rolling(20).mean().iloc[-1]
            if df_15m.iloc[-1]['volume'] > vol_avg * 2: result['score'] += 2; result['signals'].append("High Volume")
            ob = self.detect_order_blocks(df_15m, is_bullish_trend)
            if ob and is_bullish_trend and ob['type'] == 'bullish' and ob['low'] <= price <= ob['high'] * 1.01: result['score'] += 4; result['signals'].append("Order Block!")
    ...
    async def monitor_trades(self):
        ...
        for symbol in list(self.active_trades.keys()):
            ...
            if current_price <= trade['stop_loss'] and not trade['trailing_active']: closed, close_reason = True, "Stop Loss Hit"
                ...
                await self.tg(f"⚡ Trailing Active! `{symbol}`"); self._save_active_trades()
                ...
                if current_price <= trade['stop_loss']: closed, close_reason = True, "Trailing Hit!"
            if closed:
    ...
    async def scan_market(self):
        ...
        tasks = [self.analyze_coin(sym) for sym in [p['symbol'] for p in self.usdt_pairs[:30] if p['symbol'] not in self.active_trades]]
        results = await asyncio.gather(*tasks)
        signals = [r for r in results if r and abs(r['score']) >= self.MIN_SCORE_TO_TRADE]
        if signals:
            signals.sort(key=lambda x: x['score'], reverse=True)
            msg = "⚡ *Strong Signals!*\n" + "\n".join([f"🟢 `{a['symbol']}` | Score: *{a['score']}* | {a['signals'][0]}" for a in signals[:3]])
            await self.tg(msg)
            if self.TRADE_ENABLED and signals[0]['direction'] == 'BUY': await self.execute_trade(signals[0])
    ...
    async def execute_trade(self, analysis):
        ...
        if result and result.get('status') == 'FILLED':
            ...
            self._save_active_trades(); await self.tg(f"✅ BUY `{symbol}`\nEntry: `{fill_price:.4f}`")
    ...
    async def main_loop(self):
        ...
        try:
            while True:
                scan_counter += 1; await self.monitor_trades()
                if scan_counter % 900 == 0: await self.scan_market()
                await asyncio.sleep(1)
        except Exception as e: logger.critical(f"System Crash: {e}")
